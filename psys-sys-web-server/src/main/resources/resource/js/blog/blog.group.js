var _listTreeAllCodeUrl = contextPath + "/sys/code/query/listTreeAll",
	_saveOrUpdateCodeUrl = contextPath + "/sys/code/saveOrUpdate",
	_delByIdUrl = contextPath + "/sys/code/del/delById",
	_getByIdUrl = contextPath + "/sys/code/query/getById";
...
function ajaxSubAddOrModCode(treeId){
	...
	var opt = $thisForm.find("#parentId").val() == "" ? "add" : "mod";
...
function delCode(treeId, codeId){
	//同步删除资源
	var requestUrl = _delByIdUrl + "/" + codeId;
...
function getCodeById(codeId){
	var requestUrl = _getByIdUrl + "/" + codeId;
